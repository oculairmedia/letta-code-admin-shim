...
The `.d.ts` is generated by `npm run build` in `admin-shim/` and exports
`ChannelPlugin`, `ChannelAdapter`, and `ChannelHost`. See
`admin-shim/docs/CHANNEL_PLUGINS.md` for the full opt-in pattern (JSDoc
annotations + import-typeof helpers).
