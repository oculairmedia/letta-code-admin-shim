...
// lcp-vw2h: native image generation tool. Letta Code's bundled CLI already
// exposes a first-class local tool registry (`TOOL_DEFINITIONS`) and loads a
// model-specific default allowlist from ANTHROPIC_DEFAULT_TOOLS*/
// OPENAI_PASCAL_TOOLS*. Patch that registry rather than installing a skill so
// agents can call deterministic image generation through the normal tool
// pipeline, hooks, permission checks, stream frames, and local-store tool-result
// persistence. The tool talks to the local OpenAI-compatible max proxy image
// endpoint added at /v1/images/generations and saves returned b64 images to a
// predictable artifact directory before returning structured metadata plus an
// image content block.
...
function patchLettaCodeSource(raw, path, warn) {
  ...
  if (patched.includes(TOOL_DEFINITIONS_ASSIGN_TOKEN)) {
    patched = patched.replaceAll(TOOL_DEFINITIONS_ASSIGN_TOKEN, TOOL_DEFINITIONS_ASSIGN_REPLACEMENT);
    appliedPatches += 1;
