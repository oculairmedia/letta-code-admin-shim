...
// ── Native Letta Code Goal mode (read-only wrapper) ─────────────────────
// Goal mode is owned by letta.js (/goal): one conversation objective, token
// budget/usage, active time, lifecycle tools, and autonomous continuation.
// The shim must NOT maintain a parallel goals store; it only surfaces the
// native conversation-goal state persisted in .letta/settings.local.json.

function handleAgentNativeGoal(_req: IncomingMessage, res: ServerResponse, agentId: string): void {
  if (!resolveAgentRecord(agentId)) return notFound(res, `agent ${agentId}`);
  const result = getNativeGoalForAgent(agentId);
  if (!result) {
    return json(res, 200, {
...
function handleConversationNativeGoal(_req: IncomingMessage, res: ServerResponse, conversationId: string): void {
  const result = getNativeGoalForConversation(conversationId);
  if (!result) {
    return json(res, 200, {
      source: "letta_code_goal_mode",
...
async function handleAgentNativeGoalCommand(req: IncomingMessage, res: ServerResponse, agentId: string): Promise<void> {
  if (!resolveAgentRecord(agentId)) return notFound(res, `agent ${agentId}`);
  const body = await readJsonBody(req);
  const command = typeof body["command"] === "string" ? body["command"] : "";
  if (!command.trim().startsWith("/goal")) {
...
const server = createServer((req, res) => {
  ...
  const agentNativeGoalCommand = pathname.match(/^\/v1\/agents\/(agent-[^/]+)\/goal\/command\/?$/);
  if (agentNativeGoalCommand && req.method === "POST") {
    return handleAgentNativeGoalCommand(req, res, agentNativeGoalCommand[1]!);
  }
  const agentNativeGoal = pathname.match(/^\/v1\/agents\/(agent-[^/]+)\/goal\/?$/);
  if (agentNativeGoal && req.method === "GET") {
    return handleAgentNativeGoal(req, res, agentNativeGoal[1]!);
  }
  ...
  const convNativeGoal = pathname.match(/^\/v1\/conversations\/([^/]+)\/goal\/?$/);
  if (convNativeGoal && req.method === "GET") {
    return handleConversationNativeGoal(req, res, decodeURIComponent(convNativeGoal[1]!));
  }
